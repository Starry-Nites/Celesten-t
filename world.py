...
class World:
    ...
    def _setup_world(self, layout):
        ...
        for i in range(10): # Run the procgen 'n' times
        ...
        for row_index, row in enumerate(layout): # layout = world_map
            for col_index, cell in enumerate(row):
                ...
                # Make a goal anywhere the layout has a 'G'
                elif cell == 'G':
                    goal_sprite = Goal((x, y), tile_size) # attach x's and y's to the goal(s)
                    
                    self.goal.add(goal_sprite) # add the 
                elif cell == 'C':
    ...
    def _scroll_x(self):
        ...
        if player_x < WIDTH // 3 and direction_x < 0: # if the player is going right and the player is within one third of the screen size:
            ...
            player.speed = 0 # Make sure the movement doesn't look wonky when we are shifting
    ...
    def _horizontal_movement_collision(self):
        ...
        player.rect.x += player.direction.x * player.speed # Multiply the direction (usually either 1 or -1) by the speed (usually 3). Then move the player by that many pixels.
    ...
    def update(self, keys):
        ...
        self.game._show_points(self.player.sprite, self.points)
